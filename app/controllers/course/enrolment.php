...
class Course_EnrolmentController extends AuthenticatedController {
	...
	if ($courseset) {
		...
		if ($courseset->isSeatDistributionEnabled()) {
			...
			$msg_details[] = _("Zeitpunkt der automatischen Verteilung: ") . strftime("%x ", $courseset->getSeatDistributionTime());
	...
	$courseset = CourseSet::getSetForCourse($this->course_id);
	$user_id = $GLOBALS['user']->id;
	AdmissionPriority::unsetPriority($courseset->getId(), $user_id, $this->course_id);

	$this->redirect($this->url_for('/apply/' . $this->course_id));
    }

    function order_up_action($id){
	$courseset = CourseSet::getSetForCourse($this->course_id);
	$user_id = $GLOBALS['user']->id;
	$prios = AdmissionPriority::getPrioritiesByUser($courseset->getId(),$user_id);
	
	$currentPrio = (int)$prios[$id];
	$prevPrio = (int)$currentPrio-1;
	if($prevPrio < 0) $prevPrio = count($prios)-1;
	
	$reversedPrios = array_flip($prios);
	$currentCourse = $id;
	$prevCourse = $reversedPrios[$prevPrio];
	

	$reversedPrios[$prevPrio] = $currentCourse;
	$reversedPrios[$currentPrio] = $prevCourse;
	$newPrios = array_flip($reversedPrios);

	foreach($newPrios as $course_id => $prio){
	    AdmissionPriority::setPriority($courseset->getId(), $user_id, $course_id, $prio);
	}
	$this->redirect($this->url_for('/apply/' . $this->course_id));
	
    }
       function order_down_action($id){
	$courseset = CourseSet::getSetForCourse($this->course_id);
	$user_id = $GLOBALS['user']->id;
	$prios = AdmissionPriority::getPrioritiesByUser($courseset->getId(),$user_id);
	
	$currentPrio = (int)$prios[$id];
	$nextPrio = (int)$currentPrio+1;
	if($nextPrio > (count($prios)-1)) $nextPrio = 0;
	
	$reversedPrios = array_flip($prios);
	$currentCourse = $id;
	$prevCourse = $reversedPrios[$nextPrio];
	

	$reversedPrios[$nextPrio] = $currentCourse;
	$reversedPrios[$currentPrio] = $prevCourse;
	$newPrios = array_flip($reversedPrios);
