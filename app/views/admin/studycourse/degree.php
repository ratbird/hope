...
<table class="default collapsable">
    ...
    <tr class="steel">
        ...
        <td align="right">
            <a href="<?=$controller->url_for('admin/studycourse/edit_degree/'.$studydegree['abschluss_id'])?>"><?= Assets::img('icons/16/blue/edit.png', array('title' => _('Abschluss bearbeiten'), 'class' => 'text-top')) ?></a>
            <? if ($studydegree['count_user'] > 0): ?><a href="<?=URLHelper::getLink("sms_send.php?sms_source_page=sms_box.php&sd_id=".$studydegree['abschluss_id']."&emailrequest=1&subject="._("Informationen zum Studienabschluss:")." ". $studydegree['name']) ?>"><?= Assets::img('icons/16/blue/mail.png', array('title' => _('Nachricht an alle Nutzer schicken'), 'class' => 'text-top')) ?></a><? endif;?>
            <? if ($studydegree['count_user'] == 0): ?><a href="<?=$controller->url_for('admin/studycourse/delete_degree')?>?deg_id=<?= $studydegree['abschluss_id'] ?>"><?= Assets::img('icons/16/blue/trash.png', array('title' => _('Abschluss löschen'), 'class' => 'text-top')) ?></a><? endif; ?>
        </td>
    </tr>
