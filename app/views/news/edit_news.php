        ...
        <div id="news_basic_content" style="<?=$news_isvisible['news_basic'] ? '' : 'display: none'?>">
            ...
            <tbody>
                <tr>
                    <td colspan="2">
                        <label><?= _("Titel") ?><br>
                        <input type="text" name="news_topic" aria-label="<?= _('Titel der Ankündigung') ?>" value="<?= htmlReady($news['topic']) ?>"></label>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
    ...
    <div id="news_areas">
        ...
        <thead>
            <tr>
                ...
                <th><?=_('In weiteren Bereichen anzeigen')?></th>
            </tr>
        </thead>
        </table>
        <div id="news_areas_content" style="<?=$news_isvisible['news_areas'] ? '' : 'display: none'?>">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="3">
                        <label>
                        <input name="area_search_term" type="text" placeholder="<?=_('Suchen')?>" aria-label="<?= _('Suchbegriff') ?>"> 
                        <input type="image" name="area_search" src="<?= Assets::image_path('icons/16/blue/search.png')?>" aria-label="<?= _('Suche starten') ?>">
                        </label>
                    </td>
                </tr>
                <tr>
                    <td align="right">
                        ...
                        <select name="area_options_selectable[]" style="width: 200px" size="6" multiple  aria-label="<?= _('Gefundene Bereiche, die der Ankündigung hinzugefügt werden können') ?>">
                        <? foreach ($area_structure as $area_key => $area_data) : ?>
                            <? if (count($area_options_selectable[$area_key])) : ?>
                                <option disabled style="padding-left: 26px; height: 16px; font-weight: bold; color: #ffffff; 
                                        ...
                                        background-repeat: no-repeat; background-color: #d1d1d1"><?=$area_data['title']?></option>
                                <? foreach ($area_options_selectable[$area_key] as $area_option_key => $area_option_title) : ?>
                                    <option <?= StudipNews::haveRangePermission($area_option_key, 'edit') ? 'value="'.$area_option_key.'"' : 'disabled'?>><?= $area_option_title?></option>
                                <? endforeach ?>
                            <? endif ?>
                        <? endforeach ?>
                        </select></label>
                    ...
                    <td>
                        ...
                        <select name="area_options_selected[]" style="width: 200px" size="6" multiple aria-label="<?= _('Bereiche, in denen die Ankündigung angezeigt wird') ?>">
                        <? foreach ($area_structure as $area_key => $area_data) : ?>
