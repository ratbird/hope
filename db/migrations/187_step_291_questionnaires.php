...
class Step291Questionnaires extends Migration
{
    ...
    function up()
    {
        ...
        while ($vote = $all_votes->fetch(PDO::FETCH_ASSOC)) {
            ...
            $questionnaire['stopdate'] = $vote['stopdate'];
            $questionnaire['visible'] = $vote['state'] === "active" ? 1 : 0; // stopvis new active stopinvis
            ...
            foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $answer_data) {
                ...
                $answer['user_id'] = $questionnaire['anonymous'] ? null : $answer_data['user_id'];
                $answer['chdate'] = $questionnaire['anonymous'] ? 1 : $answer_data['votedate']; //damit man nicht aus dem chdate auf die user_id schließen kann
                $answer['mkdate'] = $questionnaire['anonymous'] ? 1 : $answer_data['votedate']; //mkdate genauso
                ...
                if ($questionnaire['anonymous']) {
                    $anonymous_answer = QuestionnaireAnonymousAnswer();
                    $anonymous_answer['questionnaire_id'] = $questionnaire->getId();
                    $anonymous_answer['user_id'] = $answer_data['user_id'];
                    $anonymous_answer['chdate'] = $answer_data['votedate'];
                    $anonymous_answer['mkdate'] = $answer_data['votedate'];
