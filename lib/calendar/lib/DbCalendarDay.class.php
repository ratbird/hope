...
class DbCalendarDay extends CalendarDay {
	...
	function restore ($restrictions = NULL, $sem_ids = NULL) {
 	...
 	//	$this->driver->openDatabaseGetView($this);
		...
		while ($properties = $this->driver->nextProperties()) {
			...
			$duration = (int) ((mktime(12, 0, 0, date('n', $properties['DTEND']),
			            date('j', $properties['DTEND']),
									date('Y', $properties['DTEND']), 0)
			            - mktime(12, 0, 0, date('n', $properties['DTSTART']),
									date('j', $properties['DTSTART']),
				...
				case 'MONTHLY':
					if ($rep['day']) {
						$lwst = mktime(12, 0, 0, $this->mon
								...
								+ ($this->mon - date('n', $rep['ts']))) % $rep['linterval']),
								$rep['day'], $this->year, 0);
					...
					if ($rep['day']) {
						if (date('Y', $properties['DTEND']) - date('Y', $properties['DTSTART'])) {
							$lwst = mktime(12, 0, 0, $rep['month'], $rep['day'],
									$this->year - (($this->year - date('Y', $rep['ts'])) % $rep['linterval'])
									- $rep['linterval'], 0);
	...
	function bindSeminarEvents ($sem_ids = '', $restrictions = NULL) {
		...
		$this->driver->openDatabase('EVENTS', 'SEMINAR_EVENTS', $this->getStart(),
				$this->getEnd(), NULL, $sem_ids);
	...
	function createEvent ($properties, $time_range, $lwst, $hgst) {
		...
		$start = mktime(date('G', $properties['DTSTART']), date('i', $properties['DTSTART']),
                date('s', $properties['DTSTART']), date('n', $lwst), date('j', $lwst), date('Y', $lwst));
        $end = mktime(date('G', $properties['DTEND']), date('i', $properties['DTEND']),
                date('s', $properties['DTEND']), date('n', $hgst), date('j', $hgst), date('Y', $hgst));
