  ...
  // Konstruktor
	...
	function restore ($restrictions = NULL, $sem_ids = NULL) {
		...
		while ($properties = $this->driver->nextProperties()) {
			...
			$lwst = mktime(12, 0, 0, date('n', $properties['DTSTART']),
				...
				case 'WEEKLY' :
					...
					while ($adate >= $properties['DTSTART'] && $adate <= $rep['expire'] && $adate <= $end) {
						...
						for ($i = 0; $i < strlen($rep['wdays']); $i++) {
							...
							$hgst = $lwst + $duration * 86400;
							if ($lwst < $start_ts) $lwst = $start_ts;
							$wdate = $lwst;
					...
					for ($amonth = $bmonth - $rep['linterval']; $amonth <= $bmonth; $amonth += $rep['linterval']) {
						if ($rep['ts'] < $start) {
							...
							if (!$rep['day']) {
								...
								$lwst = mktime(12, 0, 0, $amonth
												- ((($year - date('Y', $rep['ts'])) * 12
												+ ($amonth - date('n', $rep['ts']))) % $rep['linterval']),
												$rep['day'], $year, 0);
	...
	function bindSeminarEvents ($sem_ids = '', $restrictions = NULL) {
		...
		$this->driver->openDatabase('EVENTS', 'SEMINAR_EVENTS', $this->getStart(), $this->getEnd(),
				NULL, $sem_ids);
		...
		while($properties = $this->driver->nextProperties()){
			...
			$adate = mktime(12, 0, 0, date("n", $properties['DTSTART']),
					date("j", $properties['DTSTART']), $this->year, 0);
			$this->appdays["$adate"]++;
			$bind = TRUE;
		}
