...
* @version	$Id: CalendarParserICalendar.class.php,v 1.2 2008/12/23 09:50:34 thienel Exp $
* @access		public
	...
	function parse ($data, $ignore) {
		...
		if (!preg_match('/BEGIN:VCALENDAR(\r\n|\r|\n)([\W\w]*)END:VCALENDAR\r?\n?/', $v_calendar, $matches)) {
			$_calendar_error->throwError(ERROR_CRITICAL,
					_("Die Import-Datei ist keine g&uuml;ltige iCalendar-Datei!"));
			return FALSE;
		}
					...
					case 'FREQ' :
						switch (trim($match[2])) {
							...
							case 'YEARLY' :
								...
								break;
							default:
								$_calendar_error->throwSingleError('parse', ERROR_WARNING,
										_("Der Import enth&auml;lt Kalenderdaten, die Stud.IP nicht korrekt darstellen kann."));
								break;
						}
						break;
