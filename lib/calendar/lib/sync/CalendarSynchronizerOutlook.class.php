<?
/**
* CalendarSynchronizerOutlook.class.php
* 
*
* @author		Peter Thienel <pthienel@web.de>, Suchi & Berg GmbH <info@data-quest.de>
* @version	$Id: CalendarSynchronizerOutlook.class.php,v 1.2 2009/07/03 10:47:35 anoack Exp $
* @access		public
...
class CalendarSynchronizerOutlook {
	...
	function CalendarSynchronizerOutlook (&$import, &$export) {
			...
			while ($ex = array_pop($events)) {
				...
				if ($ex->properties['UID'] == $in->properties['UID']) {
					if ($ex->properties['LAST-MODIFIED'] > $this->last_sync) {
						...
						$int[] = $ex;
					} elseif ($in->properties['LAST-MODIFIED'] > $this->last_sync) {
						$ext[] = $in;
					}
					$in_changed = FALSE;
				}
		...
		foreach ($events as $event) {
			if (FALSE === strpos($event->properties['UID'], 'Stud.IP-SEM-')) {
				if ($event->properties['LAST-MODIFIED'] >= $this->last_sync) {
					...
					$event->properties['LAST-MODIFIED'] = 0;
					$ext[] = $event;
					$create_export = TRUE;
				}
			} else {
