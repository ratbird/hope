...
* @version		$Id: export.inc.php,v 1.3 2009/07/03 10:47:35 anoack Exp $
* @access		public
...
if (($expmod != 'exp' && $expmod != 'imp' && $expmod != 'sync') || ($expmod == 'exp' && !empty($err))) {
	...
	if ($expmod == 'syncrec') {
		...
		$params['content'] = '';
		if ($_calendar->checkPermission(CALENDAR_PERMISSION_OWN)) {
			$tooltip = _("Es werden nur Termine von Projekten exportiert, die zuvor im Menüpunkt \"Projekttermine\" ausgewählt wurden.");
			$params['content'] = _("Bitte w&auml;hlen Sie, welche Termine exportiert werden sollen:") . "</font></div>\n"
					...
					. ">" . _("Nur Projekttermine") . "</option>\n"
					. "<option value=\"ALL\"" . ($extype == 'ALL' ? 'selected="selected"' : '')
					. ">" . _("Alle Termine") . "</option>\n</select>"
					. "&nbsp;&nbsp;&nbsp;<img src=\"{$GLOBALS['ASSETS_URL']}images/info.gif\"" . tooltip($tooltip, TRUE, TRUE) . ">\n"
					. "<br>&nbsp;\n";
		} else {
		...
		$params['content'] .= "<div><font size=\"-1\">"
		...
		if (!$exstartday) $exstartday = date("d", time());
		if (!$exstartmonth) $exstartmonth = date("m", time());
		if (!$exstartyear) $exstartyear = date("Y", time());
		...
		$text_exstart = " &nbsp <input type=\"text\" name=\"exstartday\" size=\"2\" maxlength=\"2\" value=\""
				...
				. "&nbsp;&nbsp;"
				. "<img src=\"{$GLOBALS['ASSETS_URL']}images/popupkalender.gif\" border=\"0\" "
		...
		if (!$exendday) $exendday = date("d", time());
		if (!$exendmonth) $exendmonth = date("m", time());
		if (!$exendyear) $exendyear = date("Y", time()) + 1;
		...
		$text_exend =  " &nbsp; <input type=\"text\" name=\"exendday\" size=\"2\" maxlength=\"2\" value=\""
				...
				. "', 'InsertDate', 'dependent=yes, width=210, height=210, left=500, top=150')\">";
		$params['content'] .= ">\n&nbsp;" . sprintf(_("Nur Termine vom:%sbis zum:%s"), 
				$text_exstart, $text_exend);
		...
		if ($_calendar->checkPermission(CALENDAR_PERMISSION_OWN)) {
			...
			$params['content'] = _("Sie k&ouml;nnen Termine importieren, die sich in einer iCalendar-Datei befinden.")
					...
					. "&nbsp;&nbsp;&nbsp;<img src=\"{$GLOBALS['ASSETS_URL']}images/info.gif\"" . tooltip($tooltip1, TRUE, TRUE) . ">\n"
					. "<br>&nbsp; &nbsp; <input type=\"checkbox\" name=\"import_sem_imp\" value=\"1\">\n&nbsp;"
					. _("Termine aus abonnierten Veranstaltungen importieren.")
					. "&nbsp;&nbsp;&nbsp;<img src=\"{$GLOBALS['ASSETS_URL']}images/info.gif\"" . tooltip($tooltip2, TRUE, TRUE) . ">\n"
...
elseif ($expmod == 'exp' && empty($err)) {
	...
	$export = new CalendarExportFile(new CalendarWriterICalendar());
	if ($experiod != 'period') {
		$export->exportFromDatabase($_calendar->getUserId(), 0, CALENDAR_END, $extype,
		    Calendar::getBindSeminare($_calendar->getUserId()));
	}	else {
		$export->exportFromDatabase($_calendar->getUserId(), $exstart, $exend, $extype,
		    Calendar::getBindSeminare($_calendar->getUserId()));
	}
