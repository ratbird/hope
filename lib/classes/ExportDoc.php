...
StudipAutoloader::addAutoloadPath(__DIR__ . '/exportapi/elements/');
StudipAutoloader::addAutoloadPath(__DIR__ . '/exportapi/formats/');
...
class ExportDoc extends SimpleORMap {
    ...
    public function __construct($id = null) {
        ...
        if ($this->isNew()) {
            ...
            if ($this->user_id != $GLOBALS['user']->user_id) {
                throw new AccessDeniedException(_('Keine Berechtigung dieses Template zu laden'));
            }
        }
    }
    ...
    public function loadTemplate($args) {
        ...
        // Check if we are allowed to open that template
    ...
    public function preview() {
        ...
        foreach ($this->elements as $element) {
            if ($element->isEditable()) {
                $preview[$elementCount] = $element->preview($elementCount);
            }
            $elementCount++;
        }
        return $preview;
    ...
    private function checkPermissions() {
        ...
        if ($permissions
                // and we have a context check if we have the permissions in this context
                && (($permissions->context && !$GLOBALS['perm']->have_studip_perm((string) $permissions->usertype, (string) $permissions->context))
                // if we have no context given we check the global rights
                || (!$permissions->context && !$GLOBALS['perm']->have_perm((string) $permissions->usertype)))) {
