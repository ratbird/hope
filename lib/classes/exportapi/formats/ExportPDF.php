...
class ExportPDF extends TCPDF {
    ...
    /**
     * Routine fails finding element
     */
    public function exportMissing($type) {
        $this->addContent("Export von $type wird nicht unterstützt");
    }

    public function exportText($txt) {
        $this->addContent($txt->content);
    }
    ...
    protected function convertURL($url) {
        ...
        if (strpos(implode('#', $this->domains), $url_elements['host']) !== false) {
            ...
            } else if (stripos($url, 'download') !== false || stripos($url, 'sendfile.php') !== false) {
                //// get file id
                $matches = array();
