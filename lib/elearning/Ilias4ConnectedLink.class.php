    ...
    function getAdminModuleLinks() {
        ...
        elseif ($connected_cms[$this->cms_type]->content_module[$current_module]->isAllowed(OPERATION_WRITE)) {
            ...
            if ($connected_cms[$this->cms_type]->content_module[$current_module]->isAllowed(OPERATION_COPY) AND (! in_array($connected_cms[$this->cms_type]->content_module[$current_module]->module_type, array("lm", "htlm", "sahs", "cat", "crs", "dbk")))) {
                ...
                $output .= _("Als Kopie anlegen") . "&nbsp;<img  src=\"".$GLOBALS['ASSETS_URL']."images/info.gif\" " . tooltip(_("Wenn Sie diese Option wählen, wird eine identische Kopie als eigenständige Instanz des Lernmoduls erstellt. Anderenfalls wird ein Link zum Lernmodul gesetzt."), TRUE, TRUE) . ">" . "<br>";
            }
            $output .= "<input type=\"RADIO\" name=\"write_permission\" value=\"none\" checked>";
            $output .= _("Keine Schreibrechte") . "&nbsp;<img  src=\"".$GLOBALS['ASSETS_URL']."images/info.gif\" " . tooltip(_("Nur der/die BesitzerIn des Lernmoduls hat Schreibzugriff für Inhalte und Struktur des Lernmoduls. TutorInnen und DozentInnen können die Verknüpfung zur Veranstaltung wieder löschen."), TRUE, TRUE) . ">" . "<br>";
            $output .= "<input type=\"RADIO\" name=\"write_permission\" value=\"dozent\">";
            $output .= _("Mit Schreibrechten f&uuml;r alle DozentInnen dieser Veranstaltung") . "&nbsp;<img  src=\"".$GLOBALS['ASSETS_URL']."images/info.gif\" " . tooltip(_("DozentInnen haben Schreibzugriff für Inhalte und Struktur des Lernmoduls. TutorInnen und DozentInnen können die Verknüpfung zur Veranstaltung wieder löschen."), TRUE, TRUE) . ">" . "<br>";
            $output .= "<input type=\"RADIO\" name=\"write_permission\" value=\"tutor\">";
            $output .= _("Mit Schreibrechten f&uuml;r alle DozentInnen und TutorInnen dieser Veranstaltung") . "&nbsp;<img  src=\"".$GLOBALS['ASSETS_URL']."images/info.gif\" " . tooltip(_("DozentInnen und TutorInnen haben Schreibzugriff für Inhalte und Struktur des Lernmoduls. TutorInnen und DozentInnen können die Verknüpfung zur Veranstaltung wieder löschen."), TRUE, TRUE) . ">" . "<br>";
