...
function submitWikiPage($keyword, $version, $body, $user_id, $range_id) {
    ...
    //TODO: Die $message Texte klingen f�rchterlich. Halbs�tze, Denglisch usw...
    if ($latestVersion && ($latestVersion['body'] == $body)) {
        $message = MessageBox::info(_('Keine �nderung vorgenommen.'));
...
function getZusatz($wikiData)
{
    ...
    $s .= sprintf(_(', ge�ndert von %s am %s'),
                  $user
...
function showDeleteDialog($keyword, $version) {
    ...
    if (!$perm->have_studip_perm("tutor", $SessSemName[1])) {
        throw new AccessDeniedException(_('Sie haben keine Berechtigung, Seiten zu l�schen.'));
    }
    ...
    if (!$islatest) {
        throw new InvalidArgumentException(_('Die Version, die Sie l�schen wollen, ist nicht die Aktuellste. �berpr�fen Sie, ob inzwischen eine aktuellere Version erstellt wurde.'));
    }
    $msg= sprintf(_("Wollen Sie die untenstehende Version %s der Seite %s wirklich l�schen?"), "<b>".htmlReady($version)."</b>", "<b>".htmlReady($keyword)."</b>") . "<br>\n";
    if (!$willvanish) {
        $msg .= _("Diese Version ist derzeit aktuell. Nach dem L�schen wird die n�chst�ltere Version aktuell.") . "<br>";
    } else {
        $msg .= _("Diese Version ist die derzeit einzige. Nach dem L�schen ist die Seite komplett gel�scht.") . "<br>";
    }
    ...
    $msg.='<p>'. sprintf(_("Um alle Versionen einer Seite auf einmal zu l�schen, klicken Sie %shier%s."),'<a href="'.URLHelper::getLink('?cmd=delete_all&keyword='.urlencode($keyword)).'">','</a>');
...
function showDeleteAllDialog($keyword) {
    ...
    $msg= sprintf(_("Wollen Sie die Seite %s wirklich vollst�ndig - mit allen Versionen - l�schen?"), "<b>".htmlReady($keyword)."</b>") . "<br>\n";
    if ($keyword=="WikiWikiWeb") {
...
function deleteWikiPage($keyword, $version, $range_id) {
    ...
    if (!keywordExists($keyword)) { // all versions have gone
        $addmsg = '<br>' . sprintf(_("Damit ist die Seite %s mit allen Versionen gel�scht."),'<b>'.htmlReady($keyword).'</b>');
        $newkeyword = "WikiWikiWeb";
    } else {
        $newkeyword = $keyword;
        $addmsg = "";
    }
    $message = MessageBox::info(sprintf(_('Version %s der Seite %s gel�scht.'), htmlReady($version), '<b>'.htmlReady($keyword).'</b>') . $addmsg);
    PageLayout::postMessage($message);
    if ($dellatest) {
        $lv=getLatestVersion($keyword, $SessSemName[1]);
        if ($lv) {
...
function deleteAllWikiPage($keyword, $range_id) {
    ...
    $message = MessageBox::info(sprintf(_('Die Seite %s wurde mit allen Versionen gel�scht.'), '<b>'.htmlReady($keyword).'</b>'));
    PageLayout::postMessage($message);
...
function listPages($mode, $sortby = NULL) {
    ...
    if (count($pages) == 0) {
        ...
        printf($s, 10,"center",  "<font size=-1><b><a href=\"".URLHelper::getLink("$selfurl&sortby=$versionsortlink")."\">"._("�nderungen")."</a></b></font>");
        printf($s, 15,"left",  "<font size=-1><b><a href=\"".URLHelper::getLink("$selfurl&sortby=$changesortlink")."\">"._("Letzte �nderung")."</a></b></font>");
        ...
        foreach ($pages as $page) {
            ...
            if ($mode=="new" && $version > 1) {
                print("&nbsp;(<a href=\"".URLHelper::getLink("?view=diff&keyword=".urlencode($keyword)."&versionssince=$lastlogindate")."\">"._("�nderungen")."</a>)");
            }
            print($tdtail);
...
<table class="default">
    <caption>
        <?= sprintf(_('Treffer f�r Suche nach %s'), '&raquo;' . htmlReady($searchfor) . '&laquo;') ?>
    <? if ($localsearch): ?>
...
function wikiEdit($keyword, $wikiData, $user_id, $backpage=NULL)
{
    ...
    if ($locks && $lock["user_id"]!=$user_id) {
        $message = MessageBox::info(sprintf(_("Die Seite wird eventuell von %s bearbeitet."), htmlReady($locks)), array(_("Wenn Sie die Seite trotzdem �ndern, kann ein Versionskonflikt entstehen."), _("Es werden dann beide Versionen eingetragen und m�ssen von Hand zusammengef�hrt werden."),  _("Klicken Sie auf Abbrechen, um zur�ckzukehren.")));
        PageLayout::postMessage($message);
    }
    if ($keyword=='toc') {
        $message = MessageBox::info(_("Sie bearbeiten die QuickLinks."), array(_("Verwenden Sie Aufz�hlungszeichen (-, --, ---), um Verweise auf Seiten hinzuzuf�gen.")));
        PageLayout::postMessage($message);
...
function printWikiPage($keyword, $version) {
    ...
    echo "<p><em>";
    echo sprintf(_("Version %s, letzte �nderung %s von %s."), $wikiData['version'],
...
function exportWiki() {
    showPageFrameStart();
    $message = MessageBox::info(_('Alle Wiki-Seiten werden als gro�e HTML-Datei zusammengef�gt und in einem neuen Fenster angezeigt. Von dort aus k�nnen Sie die Datei abspeichern.'));
    PageLayout::postMessage($message);
...
function getShowPageInfobox($keyword, $latest_version)
{
    ...
    if (!$latest_version) {
        $message = sprintf(_('Sie betrachten eine alte Version, die nicht mehr ge�ndert werden kann. Verwenden Sie dazu die %saktuelle Version%s.'),
    ...
    $widget->addLink(_('Standard'),
           ...
           ->setActive(true);
    if (count($versions) >= 1) {
        $widget->addLink(_('Text�nderungen anzeigen'),
...
function showWikiPage($keyword, $version, $special="", $show_comments="icon", $hilight=NULL) {
    ...
    if ($perm->have_studip_perm("autor", $SessSemName[1])) {
        if (!$latest_version) {
            ...
            $edit .= _("�ltere Version, nicht bearbeitbar!");
        } else {
            $edit="";
            if ($perm->have_studip_perm("autor", $SessSemName[1])) {
            ...
            if ($perm->have_studip_perm("tutor", $SessSemName[1])) {
                $edit.=LinkButton::create(_('L�schen'),URLHelper::getURL("?keyword=".urlencode($keyword)."&cmd=delete&version=latest"));
            }
        }
        $edit .= "<br>&nbsp;";
    } else {
    ...
    foreach ($diffarray1 as $i) {
        if (!$i || $last_author != $i->who) {
            if (trim($collect)!="") {
                ...
                echo Assets::img('icons/16/grey/info-circle.png', tooltip2(_("�nderung von").' ' . get_fullname($last_author)));
                echo "</td>";
                echo "<td><font size=-1>";
