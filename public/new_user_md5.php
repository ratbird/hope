...
if (check_ticket($_REQUEST['studipticket'])){
    ...
    while ( is_array($_POST)
        ...
        case "u_kill_x":
            ...
            $question = sprintf(_('Möchten Sie wirklich den Benutzer **%s** löschen ?'), $username);
...
if (isset($_GET['details']) || $showform ) {
    ...
    if(!$auth->auth['jscript']){
        ...
        echo "<a href=\"#\" onClick=\"\$('.pers_browse_datafields').toggle(); this.innerHTML=(!$('.pers_browse_datafields').is(':visible'))?'" . _("Erweiterte Suche") . "':'" . _("Zuklappen") . "';\">";
