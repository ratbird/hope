...
<div id="edit_entry" class="schedule_edit_entry" <?= $show_entry ? '' : 'style="display: none"' ?>>
    ...
    <!--  < ? = $controller->url_for('calendar/schedule/addentry'. ($show_entry['id'] ? '/'. $show_entry['id'] : '') ) ? > -->
    <form action="?addentry<?= $show_entry['id'] ? '&'. $show_entry['id'] : '' ?>" method="post" name="edit_entry" style="padding-left: 10px; padding-top: 10px; margin-right: 10px;" onSubmit="return STUDIP.Schedule.checkFormFields()">
        ...
        <div style="text-align: center">
            ...
            <?= LinkButton::createAccept(_('Speichern'), 'javascript:STUDIP.Schedule.addNewEntry("'.
                    PluginEngine::getLink($plugin, array(), 'addEntry')
                .'", true);') ?>
