    ...
    <?php if ($links): ?>
        <ul>
            ...
            <li><a href="<?= htmlReady($link->url) ?>" target="_blank"><?= empty($link->name) ? htmlReady($link->url) : htmlReady($link->name) ?></a>
                    ...
                    <?php if ($isDozent): ?>
                        <span><a href="edit?id=<?= $link->id ?>"><?= Assets::img('icons/16/black/comment.png', array('alt' => 'Bearbeiten')) ?></a><a href="delete?id=<?= $link->id ?>"><?= Assets::img('icons/16/black/decline.png', array('alt' => 'Löschen')) ?></a></span>
                <?php endif; ?>
