<form action="<?= URLHelper::getLink('', array('cmd' => $cmd, 'atime' => $atime)) ?>" method="post" name="jump_to">
    ...
    <img src="<?= Assets::image_path('popupcalendar.png') ?>" onClick="window.open('<?= UrlHelper::getLink("termin_eingabe_dispatch.php?element_switch=jmp&submit=1&form_name=jump_to&mcount=6&imt=$atime&atime=$atime"); ?>', 'InsertDate', 'dependent=yes, width=700, height=450, left=250, top=150')" style="vertical-align:bottom;">
