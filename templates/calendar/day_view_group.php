...
<table class="steelgroup0" width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td align="center" width="10%" height="40">
            <a href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd' => 'showday', 'atime' => $group_calendar->view->getStart() + $calendar_user_control_data['start'] * 3600 - 86400)) ?>">
                <?= Assets::img('calendar_previous.gif', tooltip2(_("zurück"))) ?>
            </a>
        </td>
        ...
        <td align="center" width="10%">
            <a href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd' => 'showday', 'atime' => $group_calendar->view->getStart() + $calendar_user_control_data['start'] * 3600  + 86400)) ?>">
                <?= Assets::img('calendar_next.gif', tooltip2(_("vor"))) ?>
            </a>
        </td>
    </tr>
    <tr>
                ...
                <tr>
                    ...
                    <? for ($i = $time + $start; $i < $time + $end; $i += 3600 * ceil($step)) : ?>
                        <td<?= $colsp ?> class="precol1w" align="center">
                            <a href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd'=> 'edit', 'atime' => $i, 'cal_group' => $group_id)) ?>" class="calhead"><?= date('G', $i) ?></a>
                        </td>
                    <? endfor ?>
                </tr>
                <? while ($calendar = $group_calendar->nextCalendar()) : ?>
                    ...
                    <tr>
                        <td width="<?= $width2 ?>%" nowrap="nowrap" class="month">
                            <span class="precol2">
                                <a class="calhead" href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd' => 'showday', 'atime' => $atime, 'cal_select' => 'user.' . get_username($calendar->getUserId()))); ?>"><?= htmlReady(($calendar->checkPermission(CALENDAR_PERMISSION_OWN) ? _("Eigener Kalender") : get_fullname($calendar->getUserId(), 'no_title_short'))) ?></a>
                            </span>
                        </td>
                        <? // display day events
                        $js_events = array(); ?>
                        ...
                        <? if ($calendar->havePermission(CALENDAR_PERMISSION_WRITABLE)) : ?>
                                <a href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd' => 'edit', 'atime' => $atime, 'devent' => '1', 'cal_select' => get_username($calendar->getUserId()))) ?>">
                        ...
                        for ($i = $start + $calendar->view->getStart(); $i < $end + $calendar->view->getStart(); $i += $group_calendar->getUserSettings('step_day_group')) :
                            ...
                            <? if ($calendar->havePermission(CALENDAR_PERMISSION_WRITABLE)) : ?>
                                <a href="<?= URLHelper::getLink($_SERVER['PHP_SELF'], array('cmd' => 'edit', 'atime' => $i, 'cal_select' => 'user.' . get_username($calendar->getUserId()))) ?>">
